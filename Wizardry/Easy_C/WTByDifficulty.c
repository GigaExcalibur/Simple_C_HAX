...
int GetWeaponTriangleHitModifier() {
	
}

int GetWeaponTriangleAtkModifier() {
...
void BattleApplyReaverEffect(struct BattleUnit* attacker, struct BattleUnit* defender) {
    ...
    if (!(attacker->weaponAttributes & IA_REVERTTRIANGLE) || !(defender->weaponAttributes & IA_REVERTTRIANGLE)) {
        attacker->wTriangleHitBonus = -(attacker->wTriangleHitBonus * 2 * difficultyHitModifier / 12);
        attacker->wTriangleDmgBonus = -(attacker->wTriangleDmgBonus * 2 * difficultyAtkModifier / 12);
        defender->wTriangleHitBonus = -(defender->wTriangleHitBonus * 2 * difficultyHitModifier / 12);
        defender->wTriangleDmgBonus = -(defender->wTriangleDmgBonus * 2 * difficultyAtkModifier / 12);
    }
}